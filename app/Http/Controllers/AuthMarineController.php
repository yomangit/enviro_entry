<?php

namespace App\Http\Controllers;

use App\Models\Biota;
use App\Models\Marine;
use Illuminate\Http\Request;
use App\Models\LocationBiota;

class AuthMarineController extends Controller
{
    public function index()
    {
       
        $grafiks= Marine::with('user')->filter(request(['fromDate','search']))->latest()->filter(request(['fromDate', 'search','location']))->paginate(10)->withQueryString();
        $taxa_richness=[];
        $species_density=[];
        $diversity_index=[];
        $evenness_value=[];
        $dominance_index=[];
        $date=[];
       foreach ($grafiks as $grafik ) {
        $date[]=date('m-Y', strtotime( $grafik->date));
        $taxa_richness[]=$grafik->taxa_richness;
        $species_density[]=$grafik->species_density;  
        $diversity_index[]=$grafik->diversity_index;
        $evenness_value[]=$grafik->evenness_value;
        $dominance_index[]=$grafik->dominance_index;
       }
        return view('Auth.Marine.index', [
            "tittle" => "Marine",
            'Biotum'=>Biota::all(),
            'LocationBiota'=>LocationBiota::all(),
            'breadcrumb' => 'Marine Monitoring',
            'date'=>$date,
            'taxa_richness'=> $taxa_richness,
            'species_density'=> $species_density,
            'diversity_index'=>$diversity_index,
            'evenness_value'=>$evenness_value,
            'dominance_index'=>$dominance_index,
            'Marine' => Marine::with('user')->latest()->filter(request(['fromDate', 'search','location']))->paginate(10)->withQueryString() //with diguanakan untuk mengatasi N+1 problem
        ]);
    }
}
